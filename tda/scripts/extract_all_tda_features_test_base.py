...
# ---------- Configuration ----------
h5_path = "../../data/lcz42/testing.h5"
output_ms_file = "../data/tda_MS_features_test.h5"
output_sar_file = "../data/tda_SAR_features_test.h5"
    ...
    with h5py.File(h5_path, "r") as f:
        ...
        if tda_ms is not None:
            with h5py.File(output_ms_file, "w") as out_f:
                out_f.create_dataset("tda_MS_features_test", data=tda_ms)
        ...
        if tda_sar is not None:
            with h5py.File(output_sar_file, "w") as out_f:
                out_f.create_dataset("tda_SAR_features_test", data=tda_sar)
            print(f"[✓] Saved SAR features → {output_sar_file} — Shape: {tda_sar.shape}, dtype={tda_sar.dtype}")

    print("\n[INFO] Feature extraction for testing completed successfully.")