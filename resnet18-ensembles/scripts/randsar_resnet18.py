...
class RandResNet18Ensemble(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for model, cfg in zip(self.models, self.member_configs):
            ...
            sar_idx = torch.tensor([cfg["sar_idx"]], dtype=torch.long, device=x.device)

            subset_ms = torch.index_select(x, dim=1, index=ms_idx)
            subset_sar = torch.index_select(x, dim=1, index=sar_idx)
...
def train_randsar_resnet18(cfgT: Dict) -> Dict:
    ...
    class _PairedDataset(Dataset):
        """Zip MS and SAR datasets so we can shuffle them with a single sampler."""

        def __init__(self, ds_ms, ds_sar):
            if len(ds_ms) != len(ds_sar):
                raise ValueError(
                    "MS and SAR datasets must share the same number of samples "
    ...
    for member_idx in range(num_members):
        ...
        band_sar = np.random.randint(0, num_bands_sar)

        bands_ms_t = torch.tensor(bands_ms, dtype=torch.long, device=device)
        band_sar_t = torch.tensor([band_sar], dtype=torch.long, device=device)

        print(f"  MS bands (1-based): {bands_ms + 1}, SAR band (1-based): {band_sar + 1}")
        ...
        for epoch in range(max_epochs):
            ...
            for images_ms, images_sar, labels in progress:
                ...
                subset_sar = torch.index_select(images_sar, dim=1, index=band_sar_t)
                subset = torch.cat([subset_ms, subset_sar], dim=1)
    ...
    ensemble = RandResNet18Ensemble(members_models, members_meta)
