...
def train_rand_fusion(cfgT: Dict) -> Dict:
    """
    Training routine for Fusion SR-based DenseNet201 (no TDA).

    Each member trains on a different SR-augmented dataset.
    Model is standard DenseNet201 (no TDA fusion).

    cfgT keys:
        dsTrain, dsTest, info
        numMembers (default 10, one per SR method)
    ...
    print(f"[INFO] Training {num_members} SR-based members (Fusion / DenseNet201).")

    for member_idx in range(num_members):
        print(f"\n[FUSION] Member {member_idx + 1:02d}/{num_members}")
        bands = np.random.choice(num_bands, size=3, replace=False)
        bands_tensor = torch.tensor(bands, dtype=torch.long, device=device)
        print(f"  Bands (1-based): {bands + 1}")
